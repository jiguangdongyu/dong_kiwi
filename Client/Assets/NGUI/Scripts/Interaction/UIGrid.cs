...
public class UIGrid : UIWidgetContainer
{
    ...
    public ECeilTweenType ceilTweenType = ECeilTweenType.None;
    public CeilTweenInfo ceilTweenInfo = new CeilTweenInfo()
    {
        ceilTweenDuration = 0.2f,
        ceilMoveInStep = 1,
        ceilTweenInterval = 0.2f
    };
    public struct CeilTweenInfo
    {
        public float ceilTweenDuration;
        public float ceilTweenInterval;     //ceill之间动作间隔
        public int ceilMoveInStep;
    }
    ...
    //扩展
    private void CeilsAppear()
    {
        /*
        if (ceilTweenType == ECeilTweenType.None)
            return;

        GridCeilTweenBase tween = null;
        if (ceilTweenType == ECeilTweenType.ScaleFadeIn)
        {
            tween = KiwiCommonFunc.AddSingleComponent<GridCeilScaleFadeIn>(gameObject);
        }
        else if (ceilTweenType == ECeilTweenType.FadeIn)
        {
            tween = KiwiCommonFunc.AddSingleComponent<GridCeilFadeIn>(gameObject);
        }
        else if (ceilTweenType == ECeilTweenType.ScaleIn)
        {
            tween = KiwiCommonFunc.AddSingleComponent<GridCeilScaleIn>(gameObject);
        }
        else if (ceilTweenType == ECeilTweenType.MoveTwoStepIn)
        {
            tween = KiwiCommonFunc.AddSingleComponent<GridCeilMoveTwoStepIn>(gameObject);
        }

        if (tween)
            tween.StartTween(ceilTweenInfo.ceilTweenDuration, ceilTweenInfo.ceilTweenInterval);

        ceilTweenType = ECeilTweenType.None;
         * */
    }
}
