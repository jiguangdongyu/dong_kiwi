...
static public class NGUIText
{
		...
		if (dynamicFont != null)
		{
			...
			if( dynamicFont.name == "msyhbd"){
				...
				if(spt==null)
				{
					...
					spt = gController.GetComponent<StartPoint>();
				}
				
				if(spt!=null)
					spt.RebuildBattleFontTexture();
                 * */
			}
		}
#endif
    ...
    static public Vector2 CalculatePrintedSize(string text, int factLength, ref float useX)
    {
        Vector2 v = Vector2.zero;
            ...
            for (int i = 0; i < textLength; ++i)
            {
                ...
                if (symbol == null)
                {
                    ...
                    if (w != 0f)
                    {
                        ...
                        if (Mathf.RoundToInt(x + w) > factLength)
                        {
                            if (x > maxX) maxX = x - finalSpacingX;
                            x = w;
                            y += finalLineHeight;
